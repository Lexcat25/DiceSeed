...
//     optimize the clear away as a dead store. The intermediate entropy
//     buffer is scoped inside diceseed_core.h and self-scrubs the moment
//     it's no longer needed, rather than living as a global until the final
//     wipe -- see diceseed_core.h for why.
...
//   groups that index into the 2048-word list. See diceseed_core.h for the
//   exact bias tradeoff this implies for the 12-word/50-roll case.
//
...
void scrubSensitiveRAM() {
  ...
  // Entropy/checksum scratch space is function-local inside
  // diceseed_core.h and zeroizes itself before returning -- there is no
  // long-lived global copy left to scrub here anymore (v1.2.0).
}
    ...
    case SCR_ROLLING: {
      ...
      if (ev == 1) {
        ...
        if (currentRollIndex >= rollsNeeded) {
          ...
          diceseed::computeMnemonic(rolls, rollsNeeded, wordCount, entBytes, csBits, mnemonicWords);
